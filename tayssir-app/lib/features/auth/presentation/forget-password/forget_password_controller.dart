...
final forgetPasswordControllerProvider = StateNotifierProvider.autoDispose<
...
class ForgetPasswordController extends StateNotifier<ForgetPasswordState> {
  ...
  void onEmailEntered(String email) async {
    ...
    if (!exists) {
      state = state.setError(
        AppException(
          type: AppExceptionType.emailNotExists,
        ),
      );
      return;
    }
  ...
  Future<void> sendOtpCode(String email) async {
    // state = state.setLoading();
    try {
      await authRepository.sendForgetPasswordOtpCode(email);
  ...
  Future<void> verifyOtpCode(String code) async {
    ...
    state = state.setLoading();
    try {
      await authRepository.verifyForgetPasswordOtpCode(state.email!, code);
  ...
  Future<void> changeForgetPassword(String password) async {
    state = state.setLoading();
    try {
      await authRepository.changeForgetPassword(
          state.email!, state.code!, password);
      // await Future.delayed(Durations.extralong4);
