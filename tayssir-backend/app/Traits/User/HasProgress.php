...
trait HasProgress
{
    ...
    {
        $chapters = $unit->chapters()->get();
        if ($chapters->isEmpty()) return 0;

        $totalProgress = 0;
        foreach ($chapters as $chapter) {
            $totalProgress += $this->chapterProgress($chapter);
        }

        return (int) ($totalProgress / $chapters->count());
    }
    ...
    public function chapterProgress($chapter): int
    {
        ...
        if ($totalQuestions === 0 && $chapter->type === 'lesson') {
            // For lessons with no questions, check if user has a bonus entry (indicating completion)
            $hasCompleted = UserChapterBonus::where('user_id', $this->id)
